...
function MatchCard({
  ...
  return (
    <div className={`group relative bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 ${
      ...
      {isPremium && (
        <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-r from-yellow-400 via-orange-500 to-yellow-400 text-white text-center py-1 text-[10px] font-black tracking-widest flex items-center justify-center gap-1 shadow-md pointer-events-none">
      ...
      {/* Menu 3 points (en dehors du lien pour ne pas déclencher la navigation) */}
      <button
        onClick={onMenuToggle}
        className="absolute top-3 right-3 w-7 h-7 bg-white/80 backdrop-blur rounded-full flex items-center justify-center hover:bg-white transition z-30"
      >
      ...
      {/* Menu déroulant */}
      {isMenuOpen && (
        <div className="absolute top-11 right-3 bg-white rounded-lg shadow-xl border border-slate-100 py-1 z-40 min-w-[140px]">
          <button
            onClick={onDelete}
      ...
      <Link href={`/discover/${match.user.id}`} className="block relative aspect-[3/4] bg-gradient-to-br overflow-hidden cursor-pointer group">
        {match.user.photoUrl ? (
          <img
          ...
          <div className={`w-full h-full flex items-center justify-center ${gradient}`}>
            <span className="text-7xl font-bold text-white/80 transition-transform duration-500 group-hover:scale-110">
        ...
        {/* Badge NOUVEAU */}
        {isNew && (
          <div className="absolute top-3 left-3 flex items-center gap-1 bg-gradient-to-r from-amber-400 to-orange-500 text-white px-2.5 py-1 rounded-full text-xs font-bold shadow-lg animate-pulse z-20 pointer-events-none">
        ...
        {/* Statut en ligne */}
        {match.user.isOnline && (
          <div className="absolute top-14 left-3 flex items-center gap-1.5 bg-white/90 backdrop-blur rounded-full px-2 py-1 z-20 pointer-events-none">
        ...
        {/* Info nom + ville */}
        <div className="absolute bottom-3 left-3 right-3 text-white z-[1] pointer-events-none">
      ...
      <div className="p-3 relative z-30">
        ...
        {match.unreadCount > 0 && (
          <div className="absolute -top-10 right-3 w-8 h-8 bg-gradient-to-br from-rose-500 to-pink-500 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-lg border-2 border-white animate-bounce pointer-events-none">
